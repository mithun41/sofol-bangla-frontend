...
import {
  Receipt, CheckCircle2, Tag,
  Percent, Minus, PauseCircle, RotateCcw, X,
} from "lucide-react";
      ...
      <div className="p-5 space-y-4">
        ...
        {totalMemberSavings > 0 && (
          <div className="flex justify-between text-xs">
            <span className="text-emerald-500 font-semibold flex items-center gap-1">
              <Tag size={10} /> Member Savings
            </span>
            <span className="font-black text-emerald-400">- {formatBDT(totalMemberSavings)}</span>
          </div>
        )}
        ...
        <div className="space-y-2">
          <p className="text-[9px] font-black uppercase tracking-widest text-slate-600">
            Discount <span className="text-slate-700 normal-case font-semibold">(F4)</span>
          </p>
        ...
        {/* ── Exchange Credit ── */}
        {exchangeCredit > 0 && (
          <div className="flex items-center justify-between bg-violet-900/30 border border-violet-700/30 rounded-xl px-3 py-2.5">
            <div className="flex items-center gap-2">
              <RotateCcw size={12} className="text-violet-400" />
              <div>
                <p className="text-[9px] font-black uppercase tracking-widest text-violet-400">Exchange Credit</p>
                <p className="font-black text-violet-300 text-sm">- {formatBDT(exchangeCredit)}</p>
              </div>
            </div>
            <button
              onClick={onClearExchange}
              className="p-1 rounded-lg hover:bg-violet-800/40 text-violet-500 hover:text-violet-300 transition-all"
        ...
        <div>
          ...
          {isMember && cartLength > 0 && totalMemberSavings > 0 && (
            <p className="text-[9px] font-semibold text-emerald-500 mt-1 flex items-center gap-1">
              <CheckCircle2 size={9} /> Saving {formatBDT(totalMemberSavings + discountAmount + exchangeCredit)} today
            </p>
          )}
        </div>
        ...
        <div className="space-y-2">
          <p className="text-[9px] font-black uppercase tracking-widest text-slate-600">
            Cash Received <span className="text-slate-700 normal-case font-semibold">(F6)</span>
          </p>
          <input
            id="cash-input"
            type="number"
