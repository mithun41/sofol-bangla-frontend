...
import { Loader2, Wallet, Users, Star, TrendingUp, Gift, Award, Crown, Zap, Sparkles } from "lucide-react";
...
export default function DashboardHome() {
  ...
  const currentMatching = user ? Math.min(user.total_left, user.total_right) : 0;
  ...
  const getNextRankInfo = () => {
    ...
    return {
      star: next.level,
      needed: next.target - currentMatching > 0 ? next.target - currentMatching : 0,
    };
  };
  ...
  const mainStats = [
    { title: "Total Balance", value: `৳ ${user.balance || "0.00"}`, icon: <Wallet size={20}/>, color: "bg-blue-600" },
    { title: "Activation PV", value: user.points || 0, icon: <Star size={20}/>, color: "bg-purple-600" },
    { title: "Left Member", value: user.total_left || 0, icon: <Users size={20}/>, color: "bg-indigo-500" },
    { title: "Right Member", value: user.total_right || 0, icon: <Users size={20}/>, color: "bg-teal-500" },
  ];

  // --- বোনাস ফান্ড কার্ডস ---
  const bonusStats = [
    { title: "Referral Bonus", value: `৳ ${user.referral_bonus || "0.00"}`, icon: <Gift size={18}/>, textColor: "text-orange-600", bgColor: "bg-orange-50" },
    { title: "Matching Bonus", value: `৳ ${user.matching_bonus || "0.00"}`, icon: <TrendingUp size={18}/>, textColor: "text-emerald-600", bgColor: "bg-emerald-50" },
    { title: "Leadership Bonus", value: `৳ ${user.leadership_bonus || "0.00"}`, icon: <Crown size={18}/>, textColor: "text-blue-600", bgColor: "bg-blue-50" },
    { title: "Rank Reward", value: `৳ ${user.rank_reward_bonus || "0.00"}`, icon: <Award size={18}/>, textColor: "text-purple-600", bgColor: "bg-purple-50" },
  ];

  // --- নতুন অফার ব্যালেন্স সেকশন (যাতে মেইন ব্যালেন্সের সাথে কনফিউশন না হয়) ---
  const offerStats = [
    { title: "Your Offer", value: `৳ ${user.total_offer_earned || "0.00"}`, icon: <Sparkles size={18}/>, textColor: "text-rose-600", bgColor: "bg-rose-50" },
    { title: "Lifetime Shopping PV", value: `${user.lifetime_offer_points || "0"} PV`, icon: <Zap size={18}/>, textColor: "text-amber-600", bgColor: "bg-amber-50" },
  ];
    ...
    <div className="p-6 bg-slate-50 min-h-screen font-sans">
      ...
      <div className="mb-8">
        <div className="flex items-center gap-3">
          ...
          <span className={`px-3 py-1 rounded-full text-[10px] font-black uppercase tracking-wider shadow-sm ${user.star_level > 0 ? "bg-yellow-400 text-yellow-950" : "bg-slate-200 text-slate-500"}`}>
        ...
        <div className="flex items-center gap-2 text-[11px] font-bold text-slate-500 uppercase tracking-wide mt-2">
          <span>Matching: <span className="text-indigo-600">{currentMatching}</span></span>
          {nextRank && (
            <>
              <span className="text-slate-300">|</span>
              <span>Need <span className="text-rose-500">{nextRank.needed}</span> more for <span className="text-amber-500">{nextRank.star} Star</span></span>
            </>
          )}
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-8">
        {mainStats.map((item, index) => (
          <div key={index} className="bg-white p-6 rounded-3xl shadow-sm border border-slate-100 transition-transform hover:scale-[1.02]">
