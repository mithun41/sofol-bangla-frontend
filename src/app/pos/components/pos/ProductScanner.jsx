...
export default function ProductScanner({
  ...
  playBeep,
  formatBDT,
  onSelectProduct, // Navigating through this handler now
}) {
  
  const onKeyDown = (e) => {
    if (e.key === "Enter") {
      e.preventDefault();
      handleScan(e); // Barcode logic handleScan-ei thakbe
    }
  };
              ...
              searchResults.map((p) => (
                ...
                >
                  <div>
                    ...
                    <p className="text-[10px] text-slate-400 font-bold">STOCK: {p.stock}</p>
                  </div>
                  <span className="font-black text-blue-600">{formatBDT(p.price)}</span>
                </div>
              ))
            )}
          </div>
